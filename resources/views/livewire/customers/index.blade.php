<div class="space-y-6">
            ...
            <div class="table-wrap ring-0">
                <table class="table">
                    <thead>
                        <tr>
                            <th class="w-10">
                                <input type="checkbox" wire:model.live="selectAll" class="checkbox" aria-label="انتخاب همه مشتریان این صفحه">
                            </th>
                            <th>مشتری</th>
                    ...
                    <tbody>
                        @foreach($this->records as $customer)
                            <tr wire:key="customer-{{ $customer->id }}">
                                <td class="w-10">
                                    <input type="checkbox" wire:click="toggleSelect({{ $customer->id }})" @checked(in_array($customer->id, $selected)) class="checkbox" aria-label="انتخاب مشتری {{ $customer->name }}">
                                </td>
                                <td>
    ...
    {{-- bulk delete confirm --}}
    <x-modal wire:model="showBulkModal" :title="'حذف ' . fa_num(count($selected)) . ' مشتری'" size="sm">
        @if(count($selected) > 0)
            <div class="space-y-4">
                <div class="flex items-center gap-3 rounded-xl bg-rose-50 p-4 text-rose-700 ring-1 ring-rose-600/10 dark:bg-rose-500/10 dark:text-rose-400 dark:ring-rose-400/20">
                    <x-icon name="alert-triangle" class="size-6 shrink-0" />
                    <p class="text-sm leading-6">
                        {{ fa_num(count($selected)) }} مشتری انتخاب‌شده به‌همراه اشتراک‌ها، لایسنس‌ها و سوابق خرید مرتبط برای همیشه حذف می‌شوند.
