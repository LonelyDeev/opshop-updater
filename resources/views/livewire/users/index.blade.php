<div class="space-y-6">
    ...
    <div class="card flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
        ...
        <div wire:loading wire:target="search, role, status, sort" class="flex items-center gap-2 text-xs text-brand-600 dark:text-brand-400">
            ...
            <div class="table-wrap ring-0">
                <table class="table">
                    <thead>
                        <tr>
                            <th class="w-10">
                                <input type="checkbox" wire:model.live="selectAll" class="checkbox" aria-label="انتخاب همه کاربران این صفحه">
                            </th>
                            <th>کاربر</th>
                    ...
                    <tbody>
                        @foreach($this->records as $user)
                            <tr wire:key="user-{{ $user->id }}">
                                <td class="w-10">
                                    @if($user->id === auth()->id())
                                        <input type="checkbox" disabled class="checkbox" title="حساب خودتان قابل حذف نیست" aria-label="حساب خودتان قابل انتخاب نیست">
                                    @else
                                        <input type="checkbox" wire:click="toggleSelect({{ $user->id }})" @checked(in_array($user->id, $selected)) class="checkbox" aria-label="انتخاب کاربر {{ $user->name }}">
                                    @endif
                                </td>
                                <td>
    ...
    {{-- bulk delete confirm --}}
    <x-modal wire:model="showBulkModal" :title="'حذف ' . fa_num(count($selected)) . ' کاربر'" size="sm">
        @if(count($selected) > 0)
            <div class="space-y-4">
                <div class="flex items-center gap-3 rounded-xl bg-rose-50 p-4 text-rose-700 ring-1 ring-rose-600/10 dark:bg-rose-500/10 dark:text-rose-400 dark:ring-rose-400/20">
                    <x-icon name="alert-triangle" class="size-6 shrink-0" />
                    <p class="text-sm leading-6">
                        {{ fa_num(count($selected)) }} کاربر انتخاب‌شده برای همیشه حذف می‌شوند.
                        حساب شما و حداقل یک مدیر همیشه باقی می‌ماند. این عمل قابل بازگشت نیست.
                    </p>
                </div>
                <div class="flex items-center justify-end gap-2">
